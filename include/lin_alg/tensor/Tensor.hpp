#ifndef _YAFEL_TENSOR_HPP
#define _YAFEL_TENSOR_HPP
...
public:
    ...
    Tensor(std::initializer_list<dataType> il) {
        ...
        //static_assert(il.size() > DIM, "Tensor: Error: Initializer list ctor size() \\neq DIM");
                      

        size_type i=0;
        auto it_end = il.end();
        for(auto it=il.begin(); it!=it_end && i<DIM; ++i, ++it) {
            (*this)(i) = *it;
        }
    ...
    template <typename T, typename dataType2>
    Tensor(const TensorExpression<T,DIM,RANK,dataType2> &tens) {
