...
constexpr auto get_last_N_impl(sequence<N>, sequence<S, SS...>)
{
    return typename std::conditional<sequence<S, SS...>::size() == N,
            sequence<S, SS...>,
            decltype(get_last_N_impl(sequence<N>(), sequence<SS...>()))>::type();
}
...
constexpr auto get_last_N(sequence<N>, sequence<SS...>)
{
    ...
    return get_last_N_impl(sequence<N>(), sequence<SS...>());
}
